class User < ActiveRecord::Base
  ...
  has_secure_password #uses BCrypt gem to create hash, #gives hashed password_digest(saved in password_digest column in db), #password & password confirmation attributes, including prescence and matching validation, #authenticate method, returns user from db when password is correct              
  validates :password, 
             presence: true, 
             length: { minimum: 6 }, 
